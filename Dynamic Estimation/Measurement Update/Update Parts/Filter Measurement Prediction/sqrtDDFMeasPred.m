...
if(algorithm==2)%Second order DDF
    ...
    for curDim=1:xDim
        Szx2(:,curDim)=gValsPlusX(:,curDim)+gValsMinusX(:,curDim)-2*zPred;
    end
    Szx2=(sqrt(deltaH^2-1)/(2*deltaH^2))*Szx2;
    
    %Equation 71 in [1] (Equation 110 in [2]) without the SR term,squared.
    PzPred=Szx*Szx'+Szx2*Szx2';
else
    Szx2=[];
    %Equation 61 in [1] (Equation 100 in [2]) without the SR term,squared.
    PzPred=Szx*Szx';
end
