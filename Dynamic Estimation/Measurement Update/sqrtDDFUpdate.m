function [xUpdate,SUpdate,innov,Szz]=sqrtDDFUpdate(xPred,SPred,z,SR,h,algorithm,innovTrans)
...
if(algorithm==2)%Second order DDF
    %Equation 115 in [2].
    SUpdate=tria([SPred-K*Szx,K*SR,K*Szx2]);
else
    %Equation 106 in [2]
    SUpdate=tria([SPred-K*Szx,K*SR]);
end
end
