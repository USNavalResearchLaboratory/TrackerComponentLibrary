function [xPred,SPred]=sqrtDDFDiscPred(xPrev,SPrev,f,SQ,algorithm)
%%SQRTDDF2DISCPRED Perform the measurement update step in the square-root
...
%comes by relaizing that the first moment with respect to the additive
%process noise is just the square root of the process noise covariance
%matrix and the second moment is zero. The central difference filter of [3]
%is the same as a first-order divided difference filter, but using a
...
for curDim=1:xDim
    fValsPlusX(:,curDim)=f(xPrev+h*SPrev(:,curDim));
    fValsMinusX(:,curDim)=f(xPrev-h*SPrev(:,curDim));
end
...
if(algorithm==2)%Second order DDF
    ...
    for curDim=1:xDim
        ...
        xPred=xPred+fValsPlusX(:,curDim)+fValsMinusX(:,curDim);
    end
    Sxx2=(sqrt(h^2-1)/(2*h^2))*Sxx2;
    xPred=((h^2-xDim)/(h^2))*f0+xPred/(2*h^2);
    
    %Equation 108 in [2]
    SPred=tria([Sxx,SQ,Sxx2]);
else
