function xNED=ECEF2NED(plhOrigin,xECEF,a,f)
%%ECEF2ENU Convert from an Earth-centered Earth-fixed (ECEF) Cartesian
%          coordinate system based to a local North-East-Down(NED)
...
%             coordinate system that should be converted to a local END
%             coordinate system.
...
%             is used.
%
%OUTPUTS: xNED A 3XnumPts matrix of the points in xECEF converted to NED
%              coordinates.
%
...
%
%EXAMPLE:
%In this example, we convert a point from the local NED coordinate system
%to ECEF and then back to the local NED cordinate system. The relative
...
% 
% xNED=[100;200;-300];
% RelErr=abs(ECEF2NED(plhPoint,NED2ECEF(plhPoint,xNED))-xNED)./xNED
%One will find the relative error to be under 1e-12, which is on the order
...
xNED(1,:)=sum(bsxfun(@times,uENU(:,2),origShiftPts),1);
xNED(2,:)=sum(bsxfun(@times,uENU(:,1),origShiftPts),1);
xNED(3,:)=sum(bsxfun(@times,-uENU(:,3),origShiftPts),1);
