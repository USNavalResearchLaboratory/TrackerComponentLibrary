function J=azEquidistantProjCartJacob(latLonPt,latLonRef,rE)
%%AZEQUIDISTANTPROJCARTJACOB Find the gradient of an azimuthal equidistance
%           projection of points on a spherical Earth with respect to the 
%           Cartesian [x;y;z] components.
%
%INPUTS: latLonPt A 2X1 [latitude;longitude] points in radians at which
...
%           position components. Each row is a component azimuthal
%           equidistant x and y in that order with derivaties of x, y, and
%           z given across the columns in that order.
%
...
% f=@(pt)ellips2AzEquidistantProj(Cart2Ellipse(pt,[],rE,0),latLonRef,rE,0);
% xyzPt=ellips2Cart(latLonPt,rE,0);
% JNumDiff=numDiff(xyzPt,f,2,[],1e-4*norm(xyzPt));
% J=azEquidistantProjCartJacob(latLonPt,latLonRef,rE);
