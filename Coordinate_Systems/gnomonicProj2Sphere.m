function latLonPts=gnomonicProj2Sphere(xy,latLonRef,r)
%%GNOMONICPROJ2SPHERE Given a point in a tangent plane with respect to
%           a point on the sphere, project the point onto the sphere using
%           a gnomonic projection. A line from the point in the plane to
%           the sphere below the plane is normal to the sphere (not the
%           plane).
%
...
%
%The conversion from Chapter 22 of [1] is implemented.
%
%EXAMPLE 1:
...
%their projections. That way, one can see that the lines hit the sphere at
%right angles.
% r=1;
% numPts=1000;
% latLonRef=[pi/4;0];
...
% numYPts=11;
% xyPts=[zeros(1,numYPts);linspace(-3*r,3*r,numYPts)];
% latLonPts=gnomonicProj2Sphere(xyPts,latLonRef,r);
...
%
%EXAMPLE 2:
%In this example, we take some points in gnomonic coordinates and convert
%then to [latitude;longitude] under a spherical Earth approximation. We
%also just compte the 3D points of the gnomonic coordinates in the local
%tangent plane, convert them to latitude;longitude, and throw out the
%height component. Both approach give the same results, within finite
%precision limits, given the the absolute error below. This just
%demonstrates that gnomonic coordinates are equivalent to a local tangent
%plane approximation that is projected onto the Earth's surface by setting
%the ellipsoidal height to 0.
% r=10;
% latLonRef=[pi/4;0];
% latLonRefCart=ellips2Cart(latLonRef,r,0);
% %Select some point in the plane and find their projections on the sphere.
% numYPts=11;
% xyPts=[zeros(1,numYPts);linspace(-3*r,3*r,numYPts)];
% %Convert with the standard function.
% latLonPts=gnomonicProj2Sphere(xyPts,latLonRef,r);
% %Take those points on the plane and convert them to unprojected global
% %coordinates.
% uENU=getENUAxes(latLonRef,false,r,0);
% xyPts3D=latLonRefCart+bsxfun(@times,xyPts(1,:),uENU(:,1))+bsxfun(@times,xyPts(2,:),uENU(:,2));
% %Convert to ellipsoidal coordinates and then throw out the height
% %component.
% llhPoints=Cart2Ellipse(xyPts3D,[],r,0);
% latLonPtsThrowOut=llhPoints(1:2,:);
% AbsErr=max(abs(latLonPtsThrowOut(:)-latLonPts(:)))
%
%REFERENCES:
...
%Equation 20-18
rho=sqrt(x.^2+y.^2);
%Equation 22-16, but we directly evaluate cos(C) and sin(C) rather than C
%directly.
tanC=rho./r;
cosC=1./sqrt(1+tanC.^2);
sinC=tanC./sqrt(1+tanC.^2);
