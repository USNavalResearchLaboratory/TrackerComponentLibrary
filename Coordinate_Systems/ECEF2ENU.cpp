...
*          system. See the Matlab implementation for more details.
*
* The algorithm can be compiled for use in Matlab  using the 
* CompileCLibraries function.
*
*The algorithm is run in Matlab using the command format
*[xENU,M]=ECEF2ENU(plhOrigin,xECEF,a,f)
*
...
void mexFunction(const int nlhs, mxArray *plhs[], const int nrhs, const mxArray *prhs[]) {
    ...
    if(mxGetM(prhs[1])!=3) {
        mexErrMsgTxt("xECEF has the wrong number of rows.");
        return;
    }
    
    const size_t numPts=mxGetN(prhs[1]);
    const double *xECEF=mxGetDoubles(prhs[1]);
    ...
    ECEF2ENUCPP(numPts, xECEF, plhOrigin, a, f, u, M);
}
