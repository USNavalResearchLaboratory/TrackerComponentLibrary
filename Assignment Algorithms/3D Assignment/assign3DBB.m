...
%   epsVal If initMethod=1, then this is the epsVal input to assign3D to
%          determine whether the function has converged in terms of the
%          relative duality gap. If initMethod~=1, then this parameter is
...
if(abs(relGap)>epsVal)
    ...
    %submatrices). This allocates more memory than is needed, because the
    %matrix shrinks as the level increases, but using matrices in Matlab is
    %fast. In C, it would be better to allocate something of the minimum
    %size.
    CostMatCur=zeros(n1,n2,n3,numLevels);
    CostMatCur(:,:,:,1)=C;%The marginal cost matrix of the first level.
    ...
    while(curLevel>0)
        if(curLevel==numLevels)
            ...
            for curTuple=1:numFreeTuples(curLevel)
                ...
                if(CostMatCur(i,j,k,curLevel)<minVal)
                    minVal=CostMatCur(i,j,k,curLevel);
                    minIdx=curTuple;
                end
            end
            if(numLevels>1)
        ...
        elseif(increasingLevelNumber==true)
            ...
            while(curTuple<=numFreeTuples(curLevel))
                ...
                if(~isfinite(CostMatCur(i,j,k,curLevel)))
                    curTuple=curTuple+1;
                    continue;
                end

                CostMatCur(1:(n1-curLevel),1:(n2-curLevel),1:(n3-curLevel),curLevel+1)=CostMatCur(2:(n1-curLevel+1),[1:(j-1),(j+1):(n2-curLevel+1)],[1:(k-1),(k+1):(n3-curLevel+1)],curLevel);
                
                if(curLevel>1)
                    boundVals(curTuple,curLevel)=cumAssignedCost(curLevel-1)+CostMatCur(i,j,k,curLevel)+assign3DLB(CostMatCur(1:(n1-curLevel),1:(n2-curLevel),1:(n3-curLevel),curLevel+1),boundType);
                else
                    boundVals(curTuple,1)=CostMatCur(i,j,k,1)+assign3DLB(CostMatCur(1:(n1-1),1:(n2-1),1:(n3-1),2),boundType); 
                end
        ...
        if(increasingLevelNumber==false)
            ...
            if(prevMinCost(curLevel)~=gain)
                ...
                %the first index where the bound is <=minCostVal.
        ...
        k=assignedTuples(2,curLevel);
        if(curLevel>1)
            cumAssignedCost(curLevel)=cumAssignedCost(curLevel-1)+CostMatCur(i,j,k,curLevel);
        else
            cumAssignedCost(curLevel)=CostMatCur(i,j,k,1);
        end
        ...
        %This has draws tuples for the next level down from
