function [A,xHyp,PHyp,GateMat]=makeStandardLRMatHyps(xPred,SPred,measUpdateFuns,zMeas,SRMeas,PD,lambda,rPred,gammaVal,measTypes,zNative,measJacob)
...
%              is not in Cartesian coordinates, then the inputs measJacob
%              and zNative must be provided.
...
%              measurement location is used. For the ith measurement, we
%              get the function for the type of measurement
%              measJacobCur=measJacob{curMeasType} and then the value
%              is det(measJacob(zNative(:,i)))*lambda(curMeasType). Thus,
%              zNative is a zOrigDimXnumMeas matrix of measurements and
%              measJacob is a numTypesX1 or 1XnumTypes cell array of
%              function handles to get the Jacobian of the measurement. If
%              the Jacobian functions are the same for all measurements,
%              then measJacob can be just a function handle. For example,
%              if the original measurements were in spherical coordinates,
%              one could use 
...
if(nargin>11&&~isempty(zNative)&&isa(measJacob,'function_handle'))
    temp=measJacob;
    measJacob=cell(numMeasUpdateFuns,1);
