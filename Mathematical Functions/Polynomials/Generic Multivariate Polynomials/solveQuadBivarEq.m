...
%solution that agrees with both quadratic equations. However, that is
...
%Normalize coefficients to reduce influence of scaling issues.
...
%If sols contains more than four solutions (the maximum allowed), then some
%of them are false. We will only keep the one that has the lowest cost and
%add others the maximize the minimum distance from the other solutions.
if(numSol>4)
    ...
    %The third solution is the one maximizing the minimum distance from the
