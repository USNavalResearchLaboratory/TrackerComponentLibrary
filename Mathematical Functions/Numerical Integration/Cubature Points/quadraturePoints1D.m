...
%           properties of orthogonal polynomials. The evaluation to a
...
%           particular degree. For high-order polynomials. other functions,
%           quadrature integration is just an approximation.
%
...
%           3 w(x)=1/sqrt(1-x^2) on (-1,1). The formula in 25.4.38 in [1]
...
%           4 w(x)=sqrt(1-x^2) on (-1,1). The formula in 25.4.40 in [1] is
...
%           5 w(x)=sqrt(x/(1-x)) on (0,1). The formula in 25.4.42 in [1] is
...
%           6 w(x)=(1-x^2)^(c1-1/2) on (-1,1) with c1>-1/2. The
%             algorithm of [2] is used with the values from Table 22.7 of
%             [1]. Formula 3 is used for the c1=0 case. This becomes
%             numerically unstable for c1 for close to but not equal to
%             zero.
%           7 w(x)=exp(-x) on (0,Inf). The algorithm of [2] is used
%             with the values from Table 22.7 of [1].
%           8 w(x)=x^c1*exp(-x) on (0,Inf). The algorithm of [2] is used
%             with the values from Table 22.7 of [1] for c1>-1.
%           9 w(x)=(1+x)^c1 on (-1,1) for c1>-1. This is a special case of
%             the Jacobi polynomials. The algorithm of [2] is used with
%             values from Table 22.7 of [1].
%          10 w(x)=x^c1 on (0,1), c1>-1. The algorithm of [2] is used. The
%             three-term recursion was derived by explicitely evaluating
%             the integrals with the desired orthogonality constraints
%             until a pattern could be identified for an arbitrary order.
%          11 w(x)=|x|^c1 on (-1,1), c1>=0 and c1 is an integer. The
%             algorithm of [2] is used. The three-term recursion was
%             derived by explicitely evaluating the integrals with the
...
%          12 w(x)=|x|^c1*exp(-x^2) on (-Inf,Inf), c1>=0 and c1 is an
%             integer. The algorithm of [2] is used. The three-term
%             recursion was derived by explicitely evaluating the integrals
...
%        c1 This is a parameter that is only used with algorithms 6, 8, 9,
%           10, and 11 as described above.
%
...
switch(algorithm)
    ...
    case 3%Weighting function w(x)=1/sqrt(1-x^2) on (-1,1) from 25.4.38 in
          %[1].
        xi=cos((2*(1:n)-1)*pi/(2*n));
        w=repmat(pi/n,[n,1]);
    case 4%Weighting function w(x)=sqrt(1-x^2) on (-1,1) from 25.4.40 in
          %[1].
        xi=cos((1:n)*pi/(n+1));
        w=(pi/(n+1))*sin((1:n)'*pi/(n+1)).^2;
    case 5%Weighting function w(x)=sqrt(x/(1-x)) on (0,1) from 25.4.42 in
          %[1].
        xi=cos((pi/2)*((2*(1:n)-1)/(2*n+1))).^2;
        w=(2*pi/(2*n+1))*xi';
    case 6%Weighting function w(x)=(1-x^2)^(c1-1/2) on (-1,1) with c1>-1/2
          %and c1~=0. Given c1=0, Formula 3 is used.
        if(c1==0)
            [xi,w]=quadraturePoints1D(n,3);
            return;
        end
        ...
        c=@(i)(i+2*c1-2)/i;
        [xi,w]=orthoPolyZerosFromRecur(n,a,b,c,mu0);
    case 7%Weighting function w(x)=exp(-x) on (0,Inf)
        mu0=1;
        a=@(i)-1/i;
        b=@(i)(2*(i-1)+1)/i;
        c=@(i)(i-1)/i;
        [xi,w]=orthoPolyZerosFromRecur(n,a,b,c,mu0);
    case 8%Weighting function w(x)=x^c1*exp(-x) on (0,Inf) for c1>-1.
        mu0=gamma(1+c1);
        a=@(i)(-1/i);
        b=@(i)(2*i-1+c1)/i;
        c=@(i)(i-1+c1)/i;
        [xi,w]=orthoPolyZerosFromRecur(n,a,b,c,mu0);
    case 9%Weighting function w(x)=(1+x)^c1 on (-1,1) for c1>-1.
        ...
        [xi,w]=orthoPolyZerosFromRecur(n,a,b,c,mu0);
    case 10%Weighting function w(x)=x^c1 on (0,1) for c1>-1
        mu0=1/(1+c1);
        a=@(k)1;
        b=@(k)(-(c1^2+(2*(k-1)+1)*c1+2*k*(k-1))/((2*(k-1)+c1)*(2*k+c1)));
        c=@(k)((k-1)^2*(c1+k-1)^2/((2*k-3+c1)*(2*k-2+c1)^2*(2*k-1+c1)));
        [xi,w]=orthoPolyZerosFromRecur(n,a,b,c,mu0);
    case 11%Weighting function w(x)=|x|^c1 on (-1,1), c1>=0 and c1 is an
           %integer.
        ...
        [xi,w]=orthoPolyZerosFromRecur(n,a,b,c,mu0);
    case 12%Weighting function w(x)=|x|^c1*exp(-x^2) on (-Inf,Inf), c1>=0
