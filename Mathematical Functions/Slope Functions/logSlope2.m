function y=logSlope2(x)
%%LOGSLOPE2  Evaluate the function 3*(log(1+x)-x+x^2/2)/x^3 avoiding
...
%          be evaluated.
%
%OUTPUTS: y The value(s) of the function 3*(log(1+x)-x+x^2/2)/x^3 at the
%           point(s) in x.
%
...
x1=x(sel);
y(sel)=3*(log1p(x1)-x1+x1.^2/2)./x1.^3;
...
x1=x(~sel);
y(~sel)=1+x1.*(-(3/4)+x1.*(3/5+x1.*(-(1/2)+x1.*(3/7+x1.*(-(3/8)+x1.*(1/3+(-(3/10)+(3*x1)./11).*x1))))));
