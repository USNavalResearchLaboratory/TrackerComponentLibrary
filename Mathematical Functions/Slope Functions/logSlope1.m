function y=logSlope1(x)
%%LOGSLOPE1  Evaluate the function -2*(log(1+x)-x)/x^2 avoiding problems at
...
%          be evaluated.
%
%OUTPUTS: y The value(s) of the function -2*(log(1+x)-x)/x^2 at the
%           point(s) in x.
%
...
x1=x(~sel);
y(~sel)=1+x1.*(-(2/3)+x1.*(1/2+x1.*(-(2/5)+x1.*(1/3+x1.*(-(2/7)+x1.*(1/4+(-(2/9)+x1./5).*x1))))));
