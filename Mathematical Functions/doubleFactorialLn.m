function val=doubleFactorialLn(x)
%DOUBLEFACTORIALLN Evaluate log(x!!), That is, the natural logarithm of the
%                 double factorial (also known as semifactorial) of x. If x
%                 is odd, then the double factorial is defined as
%                 x*(x-2)*(x-4)*...*5*3*1. If x is
...
%          integers, or 0 or -1.   
%
%OUTPUTS: val The logarithm of the double factorial of each value of x.
%
%The gamma function/factorial representations of the double factorial for
%even and odd values are taken from [1]. 
%
%Values of the doubel factorial <=50 are tabulated and the logarithm is
%taken of them. 
%
%REFERENCES:
...
for curEl=1:numEl
    ...
    if(xCur<=50)
        ...
        val(curEl)=log(table(xCur+2));
    else
        if(xCur==0||xCur==-1)
            val(curEl)=0;
        elseif(mod(xCur,2)==0)%If it is divisible by 2.
            k=xCur/2;
            val(curEl)=k*log(2)+gammaln(k+1);
        else%Assume that it is odd.
            n=(xCur+1)/2;
            val(curEl)=gammaln(xCur+2)-(n*log(2)+gammaln(n+1));
        end
    end
end
