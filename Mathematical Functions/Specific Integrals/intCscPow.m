function val=intCscPow(u,n)
%%INTCSCPOWER Evaluate the integral of csc(u)^n du.  A definite integral
...
%        n The positive integer exponent of the cosecant term.
%
%OUTPUTS: val The 1XN set of values of the integral of csc(u)^n.
%
...
if(numDim==1)%An indefinite integral
    val=indefIntCscPow(u,n);
else%A definite integral
    val=indefIntCscPow(u(2,:),n)-indefIntCscPow(u(1,:),n);
end

end

function val=indefIntCscPow(u,n)

if(n==0)
    val=u;
    return;
elseif(n==1)
    val=-log(abs(csc(u)+cot(u)));
    return;
end
...
coeffProd=1;
for k=n:-2:endVal
    val=val-coeffProd*(1/(k-1))*cotVal.*cscVal.^(k-2);
    coeffProd=coeffProd*(k-2)/(k-1);
end

if(endVal~=2)%The final integral is over csc^1    
    val=val-coeffProd*log(abs(csc(u)+cot(u)));
end
