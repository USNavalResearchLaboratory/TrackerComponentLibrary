function iqRVal=interquartileRange(x,w,dim)
%%INTERQUARTILERANGE Determine the interquartile range of the values in x.
%             This is the difference between the value in the 75th
%             percentile and the 25th, when considering them as part of a
%             scalar empirical distribution.
%
%INPUTS:   x A vector or a matrix of values.
%          w An optional matrix of weights associated with the samples in
%            x. w should have the same dimensionality as x, be all
%            positive, and  and should sum to 1 over dimension dim. If
%            omitted or an empty matrix is passed, the samples are assumed
%            to be uniformly weighted.
%        dim An optional parameter specifying the dimensions across which
%            the interquartile range is found. If this value is omitted or
%            an empty matrix is passed, then the interquartile range is
%            computed across the first non-singleton
%            dimension of x.
%
...
%then choose the next highest point.
%
%EXAMPLES:
% x=[33;-5;42;27;-28;-35;-38;-27;-27;-27;-5];
% iqRVal=interquartileRange(x)
