...
%               QPolyKal. The functions are called as FFun(T,x0,order) and
%               QFun(T,x0,order,q0), where T is the duration over which the
%               prediction is taken.
%
...
if(nargin<9||isempty(QFun))
    Q=@(T)QPolyKal(T,x0,order,q0);
else
    Q=@(T)QFun(T,x0,order,q);
end

if(nargin<8||isempty(FFun))
    F=@(T)FPolyKal(T,x0,order);
else
    F=@(T)FFun(T,x0,order);
end
