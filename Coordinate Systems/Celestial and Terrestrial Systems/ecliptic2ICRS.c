 ...
 *
 *INPUTS:     x The NXnumVec collection of vectors in the ecliptic
 *              coordinate system to convert. N can be 2, or 3. If the
 *              vectors are 2D, then they are assumed to be azimuth and
 *              elevation in radians. 3D vectors are assumed to be
 *              Cartesian position.
 ...
 *
 *OUTPUTS: xG    The vectors rotated into the ICRS. If the input was 2D
 *               azimuth and elevation, the output will be the same. If the
 *               input was Cartesian, then the output will be Cartesian.
 *
        ...
        case 0://Use the IAU 2006 model
            if(numRow==3){
            	for(curVec=0;curVec<numVec;curVec++) {
                    ...
                    iauEceq06(TT1,TT2,curAz,curEl,&curAz,&curEl);
                ...
                for(curVec=0;curVec<numVec;curVec++) {
                   ...
                   //If a 2D vector was given, just get the azimuth and elevation.
                    ...
                    iauEceq06(TT1,TT2,curAz, curEl, curOutput, curOutput+1);
                }
            }
            break;
        case 1:
            ...
            if(numRow==3){
            	for(curVec=0;curVec<numVec;curVec++) {
                    ...
                    //Perform the conversion
                    iauLteceq(julEpoch,curAz,curEl,&curAz,&curEl);
                ...
                for(curVec=0;curVec<numVec;curVec++) {
                   ...
                   //If a 2D vector was given, just get the azimuth and elevation.
                    ...
                    //Perform the rotation, saving the result in the return vector.
                    iauLteceq(julEpoch,curAz, curEl, curOutput, curOutput+1);
                }
            }
            break;
        default:
