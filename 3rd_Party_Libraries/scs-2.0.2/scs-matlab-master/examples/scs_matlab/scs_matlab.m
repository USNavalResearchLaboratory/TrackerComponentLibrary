...
if nargin==3
    ...
    %Modified by David F. Crouse to allow for a basic verbose option to
    %silence the many messages displayed for inclusion in the Tracker
    %Component Library, January 2018.
...
if use_indirect
    ...
    %Modified by David F. Crouse to allow for the message to be silenced
    %for inclusion in the Tracker Component Library, January 2018.
    if(verbose)
    disp('Factorization')
    end
    try
...
if (tau > UNDET_TOL && tau > kap) % this is different to Lieven
    %Modified by David F. Crouse to suppress the display January 2018.
    status = 'Solved';
else
    x = nan(n,1);
    y = nan(m,1);
