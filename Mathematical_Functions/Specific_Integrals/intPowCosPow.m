function val=intPowCosPow(u,n,m)
%%INTPOWCOSPOW Evaluate the integral of u^n*cos(u)^m du.  A definite
...
%
%OUTPUTS: val The 1XN set of values of the integral of u^n*cos(u)^m.
%
%This function implements formulas 6 and 7 of Section 2.631 of [1].
%
%REFERENCES:
...
if(numDim==1)%An indefinite integral
    val=indefIntPowCosPow(u,n,m);
else%A definite integral
    val=indefIntPowCosPow(u(2,:),n,m)-indefIntPowCosPow(u(1,:),n,m);
end

end

function val=indefIntPowCosPow(u,n,m)

if(mod(m,2)==0)%Use formula 6.
    m=m/2;
    ...
    for k=0:(m-1)
        a=2*(m-k);
        x=a*u;
        val=val+binomial(2*m,k)*(1/a)^(n+1)*intPowCos(x,n);
    end
    ...
    for k=0:m
        a=2*(m-k)+1;
        x=a*u;
        val=val+binomial(2*m+1,k)*(1/a)^(n+1)*intPowCos(x,n);
    end
