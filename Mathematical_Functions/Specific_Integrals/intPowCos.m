function val=intPowCos(u,n)
%%INTPOWCOS Evaluate the integral of u^n*cos(u) du.  A definite integral
...
%          cosine term.
%
%OUTPUTS: val The 1XN set of values of the integral of u^n*cos(u).
%
...
if(numDim==1)%An indefinite integral
    val=indefIntPowCos(u,n);
else%A definite integral
    val=indefIntPowCos(u(2,:),n)-indefIntPowCos(u(1,:),n);
end

end

function val=indefIntPowCos(u,n)
...
coeffProd=1;
for k=n:-2:endVal
    %Do a cosine step and a sine step together!!!
    val=val+coeffProd*u.^k.*sinVal;
    coeffProd=-coeffProd*k;
    val=val-coeffProd*u.^(k-1).*cosVal;
    coeffProd=coeffProd*(k-1);
end

if(endVal==2)
    %The final integral is over u^0*cos(u)
    val=val+coeffProd*sinVal;
else%The final integral is over u^1*cos(u)
    val=val+coeffProd*(cosVal+u.*sinVal);
end
end
