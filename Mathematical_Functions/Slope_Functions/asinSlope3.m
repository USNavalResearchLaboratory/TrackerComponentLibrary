function y=asinSlope3(x)
%%ASINSLOPE3 Evaluate the function 6*(asin(x)-x)/x^3 avoiding problems at
...
%          be evaluated.
%
%OUTPUTS: y The value(s) of the function 6*(asin(x)-x)/x^3 at the point(s)
%           in x.
%
...
x1=x(sel);
y(sel)=6*(asin(x1)-x1)./x1.^3;

%For smaller magnitude values, use an eight-order Taylor series expansion.
%The relative error at 10^-2 is on the order of 1e-21, which is less than
%the precision of a double floating point number at that value. The series
%is given in Horner form.
x1=x(~sel);
y(~sel)=1+x1.^2.*(9/20+x1.^2.*(15/56+x1.^2.*(35/192+(189*x1.^2)/1408)));
