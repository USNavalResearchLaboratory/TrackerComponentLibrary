function Z=scatterMat2ImpedMat(S,zPortImped)
%%SCATTERMAT2IMPEDMAT Convert a scattering matrix to an impedance matrix
...
%
%INPUTS: S An NXN complex scattering matrix. The entries of this are
%          unitless. The value in S(i,j) is b(i)/a(j), where b is a length
%          N vector of power wave amplitudes exiting the ports in the
%          network and a is a length N vector of the power wave amplitudes
%          entering the ports. The amplitudes, with units sqrt(Watts) are
%          defined below. If the network is lossless and doesn't impart any
%          gain, then S*S'=the identity matrix. It is required that
%          eye(N,N)-S be invertible.
...
%
%OUTPUTS: Z The NXN impedance matrix of the network corresponding to the
%           scattering matrix. The units will typically be Ohms.
%
...
Z0sqrtR=diag(sign(real(zPortImped)).*sqrtRezPort);
Z0sqrtRInv=diag(sign(real(zPortImped))./sqrtRezPort);

Z=Z0sqrtRInv*((diag(conj(zPortImped))+diag(zPortImped)*S)/(eye(N,N)-S))*Z0sqrtR;
